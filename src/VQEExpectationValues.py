...
class VQEExpectationValues(ExpectationValues):  
    """
    Optimizes VQE with any number of layers and calculates correlations for QIRO
    """
    def __init__(self, problem: Problem, reps=1, seed=100, logging=True, correlations_best = False):
        """
        Parameters:
            problem: Problem that should be optimized
            reps: Number of VQE layers
        ...
        self.ansatz = TwoLocal(self.operator.num_qubits, 'ry', 'cx', 'circular' #linear
                                      , reps=self.reps#reps = 1
                                      , insert_barriers=True)

        self.params = np.zeros(reps * 2)

        # Set the type of the expectation value
        self.type = "VQEExpectationValue"
    ...
    def optimize(self):
        """
        Optimizes VQE parameters to minimize the energy.
        """
        ...
        vqe = SamplingVQE(self.sampler, ansatz=self.ansatz, optimizer=NFT(maxiter=1000), callback=self.build_callback(), initial_point=init_params)
        start_time = time.time()
        result = vqe.compute_minimum_eigenvalue(self.operator)
    ...
    def get_most_likely_state(self, dist):
        """
